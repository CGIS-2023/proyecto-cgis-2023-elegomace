    ...
    <x-slot name="header">
        <nav class="font-semibold text-xl text-gray-800 leading-tight" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex">
                ...
                <li>
                    <a href="#" class="text-gray-500" aria-current="page">Editar baja</a>
                </li>
            </ol>
        </nav>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('bajas.update', $baja->id) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <x-input id="fecha_hora" class="block mt-1 w-full"
                                     type="datetime-local"
                                     name="fecha_hora"
                                     :value="$baja->fecha_hora->format('Y-m-d\TH:i:s')"
                                     required />
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            @isset($paciente)
                                ...
                                <x-input class="block mt-1 w-full"
                                         type="text"
                                         disabled
                                         value="{{$paciente->user->name}} ({{$paciente->dni}})"
                                />
                            @else
                                <x-select id="paciente_id" name="paciente_id" required>
                                    <option value="">{{__('Elige un paciente')}}</option>
                                    @foreach ($pacientes as $paciente)
                                        <option value="{{$paciente->id}}" @if ($baja->paciente_id == $paciente->id) selected @endif>{{$paciente->user->name}} ({{$paciente->dni}})</option>
                                    @endforeach
                                </x-select>
                            @endisset
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            @isset($medico)
                                ...
                                <x-input class="block mt-1 w-full"
                                         type="text"
                                         disabled
                                         value="{{$medico->user->name}} ({{$medico->nombre}})"
                                />
                            @else
                                <x-select id="medico_id" name="medico_id" required>
                                    <option value="">{{__('Elige un médico')}}</option>
                                    @foreach ($medicos as $medico)
                                        <option value="{{$medico->id}}" @if ($baja->medico_id == $medico->id) selected @endif>{{$medico->user->name}} ({{$medico->nombre}})</option>
                                    @endforeach
                                </x-select>
                            @endisset
                        </div>
