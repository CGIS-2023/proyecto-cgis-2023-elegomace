    ...
    <x-slot name="header">
        <nav class="font-semibold text-xl text-gray-800 leading-tight" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex">
                ...
                <li>
                    <a href="#" class="text-gray-500" aria-current="page">Crear baja</a>
                </li>
            </ol>
        </nav>
    </x-slot>
                        ...
                        <div class="mt-4">
                            ...
                            <x-select id="paciente_id" name="paciente_id" required>
                                ...
                                @foreach ($pacientes as $paciente)
                                    <option value="{{$paciente->id}}" @if (old('paciente_id') == $paciente->id) selected @endif>{{$paciente->user->name}} ({{$paciente->dni}})</option>
                                @endforeach
                            </x-select>
                            @endisset
                        </div>
                            ...
                            @else
                                <x-select id="medico_id" name="medico_id" required>
                                    ...
                                    @foreach ($medicos as $medico)
                                        <option value="{{$medico->id}}" @if (old('medico_id') == $medico->id) selected @endif>{{$medico->user->name}} ({{$medico->nombre}})</option>
                                    @endforeach
                                </x-select>
                            @endisset
                        </div>
